...
class ApplicationWindow(QtGui.QMainWindow):
    def __init__(self):
        ...
        self.textbox_x=customLineEdit(self)
        self.textbox_y=customLineEdit(self)
        self.textbox_z=customLineEdit(self)
        sc = MplPlot3dCanvas(self.main_widget)
        button = QtGui.QPushButton('Plot', self)
        button.clicked.connect(self.handleButton)
        l.addWidget(sc)
        l.addWidget(self.textbox_x)
        l.addWidget(self.textbox_y)
        l.addWidget(self.textbox_z)
        l.addWidget(button)
        self.sc=sc
    ...
    def handleButton(self):
        #foo=sympy.sympify(changed)
        foo_x=sympy.sympify(str(self.textbox_x.text()))
        foo_y=sympy.sympify(str(self.textbox_y.text()))
        foo_z=sympy.sympify(str(self.textbox_z.text()))
        print(foo_x,foo_y,foo_z)
        sympy_p1 = sympyPlot.plot3d_parametric_line(foo_x,foo_y,foo_z,(t,-4,4),show=False,ax=self.sc.ax)
        sympy_p1.plotNow()
